	...
	function initStoreData(param){
		jsonAjax('/'+language+'/exclude/AD/queryForStore.do',param,function(res){
			...
			if(count>0){
				...
				for(var i=0;i<data.length;++i){
					html += '<li>'+
								...
								'<div class="a2 text-c font12">'+
									...
									'<p class="p6">'+
										'<span class="dis-in"><b class="col-999">设备编号：</b>'+data[i].device_id+'</span>'+
										'<span class="dis-in"><b class="col-999">广告时长：</b>'+data[i].ad_length+'秒</span> '+
										'<span class="dis-in"><b class="col-999">播放次数：</b>'+data[i].allPlayNumber+'次</span> '+
										'<span class="dis-in"><b class="col-999">价格：</b>$'+data[i].price+'元/天</span> '+
										'<span class="dis-b"><b class="col-999">空闲时段：</b>'+data[i].play_begin_time+'-'+data[i].play_end_time+'</span>'+
									'</p>'+
									'<p class="p7">'+
										function(pic){
											var pics = [];
				...
				storeDetail();
				// 分页
				pageNumber(count,param,initStoreData,ADCount,'store');
			}
		},function(){})
	}
