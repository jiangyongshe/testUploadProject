...
public class ADCommissionDao extends BaseDao<Tb_advertiser_commission, AdvertiserCommissionDto> implements Constant {
	 ...
	 * @param money
	 */
	public List<Tb_Bank_Card_Info> queryBankCardInfo(String accoutId,String cardNo,int cardType,int userType) throws Exception{
		...
		if(userType!=0){
			...
			sql=sql+"and A.account_type=? ";
			obj2[obj.length]=userType;
			obj=obj2;
		}
		sql=sql+" order by id desc";
		logWriteUtil.writeLog(LOGTYPE_DBQUERY, "sql:"+sql+";"+"param accountId:"+accoutId+"===cardNo:"+cardNo+"===userType:"+userType, LOGLEVEL_INFO, ADCommissionDao.class);
