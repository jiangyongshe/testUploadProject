...
public class CommissionDetailDao extends BaseDao<Tb_commission_detail, CommissionDetailDto> implements Constant {
	...
	public List<ADCommissionDetailDto> queryADCommissionDetail(ADCommissionDetailDto param){
		...
		params.add(param.getAccount_id());
		StringBuffer sql = new StringBuffer("select A.BALANCE_DATETIME,A.ADVERTISER_COMM,A.type from (SELECT d.referrals_type type, d.BALANCE_DATETIME,d.referrals_comm ADVERTISER_COMM "
				+ "FROM tb_commission_detail d where referrals_id=? and d.ADVERTISER_COMM>0 "
				+ ""
				+ " union all select 1 type,h.AUDIT_DATETIME BALANCE_DATETIME,(case when h.ADJUST_TYPE=1 then -h.AMOUNT when h.ADJUST_TYPE=2 then h.AMOUNT end) ADVERTISER_COMM "
				+ "from tb_fund_hedging h where h.AUDIT_STATUS=2 and h.ACCOUNT_TYPE=5 and h.ACCOUNT_ID=?) A where 1=1 ");
		// 判断是否根据开始时间查询
	...
	public int queryADCommissionDetailCount(ADCommissionDetailDto param){
		...
		params.add(param.getAccount_id());
		params.add(param.getAccount_id());
		StringBuffer sql = new StringBuffer("select count(1) from(SELECT d.referrals_type type,d.BALANCE_DATETIME,d.ADVERTISER_COMM FROM tb_commission_detail d where referrals_id=? and d.ADVERTISER_COMM>0"
				+ " union all select 1 type, h.AUDIT_DATETIME BALANCE_DATETIME,(case when h.ADJUST_TYPE=1 then -h.AMOUNT when h.ADJUST_TYPE=2 then h.AMOUNT end) ADVERTISER_COMM from tb_fund_hedging h where h.AUDIT_STATUS=2 and h.ACCOUNT_TYPE=5 and h.ACCOUNT_ID=?) A where 1=1 ");
		// 判断是否根据开始时间查询
		if(startTime!=null&&!"".equals(startTime)){
			sql.append("AND DATE_FORMAT(A.BALANCE_DATETIME,'"+MYSQL_DATE_FORMAT_YMD+"')>=DATE_FORMAT(?,'"+MYSQL_DATE_FORMAT_YMD+"') ");
			params.add(startTime);
