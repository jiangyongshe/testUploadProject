 ...
 */
var tipCloseTimeout;
function showJudegTip(status,content,hideCloseBtn,time){
	if(!time){
		time=1500;
	}
	...
	tipCloseTimeout=setTimeout(hideJudegTip,time);
}
				...
				//后台定位
				  iosFindIp();
				   /* function myFun(result){
						...
						$(".currCity").text(cityName.replace("市",""));
					}
...
//ios ip定位
function iosFindIp(){
	jsonAjax('/'+language+'/user/getUserIpForLocation.do',{},function(res){
		var str=JSON.parse(res.data);
		if(!str.status){
			$(".currCity").text(str.content.address.replace("市",""));
		}else{
			$(".currCity").text("深圳");
		}
	},function(){
...
function showNoticeDiv(){
	...
	upNoticeSessionMaxId(noticeArr[0].id);//更新库状态
	//上一条
	...
	$(".next").bind("click",function(){
		...
		if(currIndex>=noticeArr.length-1){//已到最后一条
			...
			$(".next").addClass("disabled");//禁用下一条按钮
		}
	});
}
...
function advertiseSearchMethod(isInit){
	...
	if(province&&province.indexOf("省份")<0){
		addr+=province;
	}
	if(city&&city.indexOf("城市")<0){
		addr+="*"+city;
	}
	if(area&&area.indexOf("区/县")<0){
		addr+="*"+area;
	}
	if(town&&town.indexOf("商圈")<0){
		addr+="*"+town;
	}
