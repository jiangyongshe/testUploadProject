	...
	public RuturnMessageDto walletRecharge(CustomerWalletFlow dto,HttpServletRequest req) throws Exception{
		...
		if(ALIPAY_TYPE1.equals(payType)){
			...
			if(bankCardInfo==null){
				...
				bankCardInfo.setAccount_type(ADWithdrawController.changeUserType(req.getSession().getAttribute(SESSION_USER_TYPE).toString()));
			...
			UnPayOpenAndConsumeDto unionPayDto=new UnPayOpenAndConsumeDto(bankCardInfo,isFirst,dto.getSmsCode(),basePath+"/"+MOBILE_ALIPAY_RETURN_URL+"?tNo="+tb_client_in_out_money.getSerial_number());
