...
;(function(root, factory) {
        ...
        if(!isShow){
            ...
            if (jetrigger) {
                that.valCell.on(jetrigger, function (ev) {
                    ev.stopPropagation();
                    that.currIndex=this;
                    if($(this).hasClass("bgDate")){
                    	queryAdvertiseDateForSellOut(this);
                    };
    ...
    jedfn.setValue = function (fnStr,matStr,bool) {
        var that = this, elCell=$(that.currIndex)/*elCell = that.valCell*/,strVal;
        ...
        addBgDateAndEdDateLs(that.currIndex);//选择日期改变则相应总计费用也跟着改变
        return vals;
    };
    //获取日期值
    ...
    jedfn.eachDays = function(ys, ms,ds, opts, boxCell){
    	opts.valiDate=filterDate(ys,ms);
    ...
    jedfn.dateOrien = function(boxCls, valCls, pos) {
        var that = this, tops, leris, ortop, orleri,
        	...
        	rect = that.currIndex&&that.fixed?that.currIndex.getBoundingClientRect():that.fixed ? valCls[0].getBoundingClientRect() : boxCls[0].getBoundingClientRect(),
