...
public class ClientInOutMoneyDao extends BaseDao<Tb_client_in_out_money, ClientInOutMoneyDto> implements Constant {
	...
	public List<ClientInOutMoneyDto> queryDetail(InOutMoneyParamDto param){
		...
		if(param.getAccount_type()==FOUR){//代理
			params.add(FOUR);
		}else if(param.getAccount_type()==FIVE){//业务员
			params.add(THREE);
		}else if(param.getAccount_type()==TWO){//店主
			params.add(FIVE);
		}
		StringBuffer sql = new StringBuffer(
			    "SELECT A.AMOUNT,A.STATUS,A.OPEN_DATE,(case when A.ACCOUNT_TYPE=5 then 1 when A.ACCOUNT_TYPE=3 then 2 when A.ACCOUNT_TYPE=4 then 3 end) account_type FROM tb_client_in_out_money A "
			   +"WHERE A.IN_OUT_TYPE='1' AND A.ACCOUNT_ID = ? and A.ACCOUNT_TYPE=? ");
		// 判断是否根据开始时间查询
