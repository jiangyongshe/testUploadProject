...
public class CreateADRecommend extends WebApplicationObjectSupport implements Constant{
	 ...
	 * 模板名称
	 */
	private static final String FTL_NAME_MOBILE = "mobileHotAD.ftl";
	...
	private static final String HTML_NAME_MOBILE = File.separator+"chinese"+File.separator+"templeteHTML"+File.separator+"mobileHotAD.html";
	...
	public void execute(){
		try {
			...
			// 获取模板
			Template tMB = configuration.getTemplate(FTL_NAME_MOBILE);
			//获取模板
			//Template t1 = configuration.getTemplate(FTL_NAME_BUSINESS);
			// 输出流
			Writer writer = new OutputStreamWriter(new FileOutputStream
					(ContextLoader.getCurrentWebApplicationContext().getServletContext().getRealPath("")+HTML_NAME_MOBILE),"UTF-8");
...
//			
			logWriteUtil.writeLog(LOGTYPE_TIMER, "create　html path:"+ContextLoader.getCurrentWebApplicationContext().getServletContext().getRealPath("")+HTML_NAME_MOBILE, LOGLEVEL_INFO, CreateADRecommend.class);
			// 封装参数
			Map<String,Object> map = new HashMap<String,Object>();
			map.put("data", list);
			// 产出文件
			tMB.process(map, writer);
