...
private static final LogWriteUtil logWriteUtil = LogWriteUtil.getSingleton();
	...
	public CartDto findShoppCartByAccountId(String accountId,Integer adId,String deviceId,String timeInterval) {
		...
		List<CartDto> list = getJdbcTemplate().query(sql, rowmapper,accountId,adId,deviceId,timeInterval.split("-")[0].length()==5?timeInterval.split("-")[0]+":00":timeInterval.split("-")[0],timeInterval.split("-")[1].length()==5?timeInterval.split("-")[1]+":00":timeInterval.split("-")[1]);
