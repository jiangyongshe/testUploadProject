...
(function(){
	var Provider, nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sCellFileTemplate = '<b class="stream-uploading-ico"></b>' +
					...
					'	<span class="stream-cell-info" style="display:none;"><em class="stream-filetoken"></em></span>' +
					'</div>',
	...
	StreamUploader.prototype = {
		...
		retry: function(){
            //this.retriedTimes++;
            console.info(' retry ,retriedTimes '+( this.retriedTimes))
            var g = this;
		...
		uploadEventHandler: function(event){
			...
			switch(event.type){
				...
				case "error":
				    this.retriedTimes++;
				     console.info('this.retryTimes '+this.retryTimes+' this.retriedTimes '+this.retriedTimes)
	...
	Main.prototype = {
		...
		renderUI : function(file_id) {
			var file = this.uploadInfo[file_id].file;
			...
			/// this.getNode("stream-filetoken", cellInfosNode).innerHTML = '';
			...
			openWaitAnimation("正在解析视频,请稍后...");
			 browserMD5File(file.file, function (err, md5) {
			     filetoken=md5;
			   /// this.getNode("stream-filetoken", cellInfosNode).innerHTML = filetoken;
			    $("#upload_start").attr("disabled",false); 
			    closeWaitAnimation();
			  });
		},
		bindUI : function(file_id) {
			...
			/** var fileName = this.uploadInfo[file_id].file.file.name;
			 var fileType=fileName.substring(fileName.lastIndexOf(".")+1);
			if(fileType.trim().toLowerCase()!="mp4"){
			  cancelBtn.click();
			}**/
		...
		upload : function(index) {
			...
			xhr.onreadystatechange = function() {
			    ...
			    var token, server,message,success;
				try {
					try {
					...
					if(message=='10001'){
					 ...
					 if(message=='10002'){
						 showJudegTip(currError[0]);
						  return;
					}
					if(message=='10003'){
						showJudegTip(currError[1]);
						return;
					}
					...
					if(message=='40002'){
			             fileName=json.name;
			             filePath=json.token;
			             old_file_name=json.oldName;
			             src=videoUrl+"/"+filePath+"/"+fileName;
			             playVedio();
			             showJudegTip("success","提示","该视频上传过，可以不再上传！");
				         return;
					}
					if (token) {
		...
		validateFile : function(uploader) {
			...
			if(!bStreaming && size > 2147483648){this.uploadError({status:100, statusText:"Flash最大只能上传2G的文件!"});return !1;}
			 ...
			 //showJudegTip("fail","提示","fileName:"+name+"fileType:"+fileType);
			if(fileType.trim().toLowerCase()!="mp4"&&fileType.trim().toLowerCase()!="mov"){
				//showJudegTip("fail","提示","fileName:"+name+"fileType:"+fileType.trim().toLowerCase());
			   showJudegTip("fail","提示","文件类型不是mp4！");
			   return  !1;
			}
