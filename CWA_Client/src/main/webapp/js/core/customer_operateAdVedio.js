var adTypeArr = 
	  [ "日用品广告", "食品饮料广告", "餐饮广告", "电子家电广告", "服装服饰广告", "化妆品广告",
		"钟表首饰广告", "教育培训广告", "文化广告", "会展广告", "商业商场广告", "娱乐广告", 
		"社会服务广告", "金融保险广告", "汽车广告", "房地产广告", "邮电通讯快递物流广告", "保健广告", 
		"旅游酒店广告", "家装建材广告", "交通运输广告", "其他" ];

var chineseError = 
	 [ "未知异常", "视频token无效", "信息填写不完整", "电子邮箱格式错误", 
	   "手机号格式错误","两次输入密码不一致" ];
...
$(function() {
	...
	// 获取页面参数
	operateType = getURLParam("type");// 类型（修改，查看，上传）
	adNo = getURLParam("adNo");// 订单号
//	fileType = getURLParam("fileType");// 文件类型
//	isInterCut = getURLParam("isInterCut");// 插播
	var strs = adNo.split(","), 
		htmls = "";
	for (var i = 0; i < strs.length; i++) {
		htmls += '<option value="' + strs[i] + '" data-fileType=' 
			  +  fileType.split(",")[i] + '>' + strs[i] + '</option>';
	}
	$(".adNo").html(htmls);
	...
	jsonAjax("/" + language + "/video/queryAdVideoInfo.do", {
		...
		if (res.data != null && res.data != []) {
			...
			if (operateType == "u") {
				...
				/*
				 * if(data.img!==null&&data.img!==""){ var
				 * ImgArray=data.img.split("<img"); for(var i=1;i<ImgArray.length;i++){
				 * //由于split 此时的数组是缺少 <img var html=createImg(ImgArray[i],2);
				 * $(".imgList").append(html); } }
				 */
			}
			if (data.file_type == 1) {
				...
				if (data.img !== null && data.img !== "") {
					...
					/*
					 * $("#imgDiv").html(data.img); $(".vedio
					 * img")[0].src=$("#imgUpl")[0].src;
					 * imgUrl=$("#imgUpl")[0].src; $(".vedio img").show();
					 */
				}
			}
...
function submit() {// file_name;file_path;play_path;
	...
	if (fileType == "1") {
		...
		if (title.length > 40) {
			$(".error").html("上传模板信息标题最大长度40个字符！");// 信息填写不完整
			startBtn("submit");
			return;
		}
		if (context.length > 700) {
			$(".error").html("上传模板信息内容最大长度700个字符！");// 信息填写不完整
			startBtn("submit");
			return;
		}
...
//	obj.order_type = isInterCut;
	jsonAjax(
			"/" + language + "/video/operateAdVideo.do",
			obj,
			function(res) {
				if (res.success) {
					$(".error").hide();
					...
					tipInfo({
						status:'success',
						content:"您的广告订单已提交成功，我们会尽快审核您的订单，您可以通过点击 <a href=\"/"
							+ language
							+ "/forward/customerOrder.do\" class=\"red\">我的广告订单</a> 查看订单状态，订单通过就可以播放了。</p>"
					});
					startBtn("submit");
...
var config = {
	...
	maxSize : 20971520,
	/** 单个文件最大上传大小 20M */
	tokenURL : "/vedioupload/tk/token.do",
	/** 根据文件名、大小等信息获取Token的URI（用于生成断点续传、跨域的令牌） */
	uploadURL : "/vedioupload/upload/getupload.do",
	/** HTML5上传的URI */
	messagerId : "i_stream_message_container",
	/** 消息显示容器的ID, 默认: i_stream_message_container */
	// /extFilters: [".mp4",".MP4"], /** 允许的文件扩展名, 默认: [],即全部允许 ".avi",
	// ".rmvb",".rm",".mpg",".mpeg",".mpe",".mkv",".mp4",".MP4",".mov"*/
	multipleFiles : false,
	/** 多个文件一起上传, 默认: false */
	autoUploading : false,
...
function createImg(imgUrl, type) {
	...
	if (type == 1) {
		html += '<img onclick="showPicPopup(this.cloneNode(true),event)" src="'
				+ imgUrl + '" style="cursor:pointer;display:;width: 100px;"/>';
	} else if (type == 2) {
		html += '<img onclick="showPicPopup(this.cloneNode(true),event)"  style="cursor:pointer;display:;width: 100px;"'
				+ imgUrl;
	}
