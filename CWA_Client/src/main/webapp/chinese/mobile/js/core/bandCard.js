...
$(function(){
	...
	// 提交按钮
	submit();
	var userType=Number(getLocalStorage('userType'));
	$(".bankCardName").addClass("disabled");
	if(userType==2){
		$(".accountType").html("<option value='5'>店主</option>");
	}else if(userType==5){
		$(".accountType").html("<option value='3'>业务员</option>");
	}else if(userType==6){
		$(".accountType").html("<option value='8'>销售员</option>");
	}else if(userType==4){
		$(".accountType").html("<option value='4'>代理</option>");
	}
})
...
function submit(){
	$('#submit').on('click',function(){
		...
		if(bankCardName==""||papersCode==""||mobile==""||bankCardCode==""||province.indexOf("请选择")>=0||city.indexOf("请选择")>=0||openAccountBankName.indexOf("请选择")>=0){
