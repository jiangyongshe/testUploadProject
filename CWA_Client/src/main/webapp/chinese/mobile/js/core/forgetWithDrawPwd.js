...
function getVerification(){
	$('#getVerification').on('click',function(){
		...
		// 发送请求
		jsonAjax('/'+language+'/customer/getDrawMobileVerification.do',{mobile:mobile},
		function(res){
			var msg = res.msg;
...
function submit(){
	$('#submit').on('click',function(){
		...
		// 发送请求
		jsonAjax('/'+language+'/user/forgetPwdToUpdate.do',{mobile:mobile,verification:verification,pwd:pwd,type:1},
		function(res){
			var msg = res.msg;
			if(msg=='10000'){
				forgetPwdPageTip(res.data.split(",")[0]+",正在前往我的翔云!");
				setTimeout(function() {
