...
private static final LogWriteUtil logWriteUtil = LogWriteUtil.getSingleton();
	...
	public int[] batchInsertOrder(final List<Object[]> batchInsertOrder)throws Exception {
		...
		try {
			updateCounts = jdbcTemplate.batchUpdate(
	 				"INSERT INTO tb_order(SERIAL_NUMBER,DEVICE_ID,ORDER_STATUS,vedio_id,ACCOUNT_ID,ACCOUNT_IP,COMMIT_TIME,PAY_TYPE,PLAY_BEGIN_TIME,PLAY_END_TIME,SHOP_NAME,TOTAL_PRICE,PRICE,RESULT,ADVERTISER_ID,old_total_price,total_day_number,order_type,file_type,if_sub,UNIFIED_SERIAL_NUMBER,UPLOAD_FILE_NUM) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
	 				batchInsertOrder);
	...
	public void updateOrderType(String orderNo) {
		String sql="update tb_order set order_status=3 where SERIAL_NUMBER=? or unified_serial_number=?";
		this.jdbcTemplate.update(sql, orderNo, orderNo);
		logWriteUtil.writeLog("", "update order order_type success! orderNo:"+orderNo, LOGLEVEL_INFO, AdvertiseOrderDao.class);
	}
	
	//删除订单
	public void deleteOrderByOrderNo(String orderNo,String accountId) {
		String sql="update tb_order set IF_SHOW=1 where (SERIAL_NUMBER=? or unified_serial_number=?) and account_id=?";
		this.jdbcTemplate.update(sql, orderNo,orderNo,accountId);
		logWriteUtil.writeLog("", "delete order order_status success! orderNo:"+orderNo+"===accountId:"+accountId, LOGLEVEL_INFO, AdvertiseOrderDao.class);
	}
