...
public class AdvertiserDao extends BaseDao<Tb_Advertiser, AdvertiserDto> implements Constant{
	...
	public List<AdvertiserDto> queryAdvertiser(AdvertiserDto advertiserDto){
		...
		StringBuffer sql = new StringBuffer(
				...
				"A.REFERRALS_ID,A.superior,A.notice_id,A.reg_status,A.partner_id,A.salesman_id,A.salesperson_id,A.comm_rate "+
	...
	public boolean updateAdvertiserPwdByAccountId(Tb_Advertiser tbAdvertiser){
		...
		advertiserDto.setOrderWay(null);
	...
	public List<AdDto> queryAD(AdDto param){
		...
		StringBuffer sql = new StringBuffer(
			  ...
			  + "     WHEN A.ORDER_STATUS='6' or A.ORDER_STATUS='8' THEN '已取消' "
			  + "     WHEN (A.ORDER_STATUS='7' or (A.ORDER_STATUS='4' AND A.order_type<>4 AND judegNowInTargetSection(A.PLAY_BEGIN_TIME,A.PLAY_END_TIME,B.BEGIN_TIME,B.END_TIME)='3')) THEN '已关闭' "
			  + "     WHEN A.ORDER_STATUS='4' AND A.order_type<>4  AND judegNowInTargetSection(A.PLAY_BEGIN_TIME,A.PLAY_END_TIME,B.BEGIN_TIME,B.END_TIME)='2' THEN '播放中' "
			  + "     WHEN A.ORDER_STATUS='4' AND A.order_type<>4  AND judegNowInTargetSection(A.PLAY_BEGIN_TIME,A.PLAY_END_TIME,B.BEGIN_TIME,B.END_TIME)='1' THEN '等待播放' "
			  + "     WHEN A.ORDER_STATUS='4' AND A.order_type=4 THEN '已停播' END ORDER_STATUS "
		...
		if(order_stutas!=null&&!"".equals(order_stutas)){
			...
			case ORDER_STUTAS_PLAYING:
				sql.append("A.ORDER_STATUS = '4' AND A.order_type<>4  AND judegNowInTargetSection(A.PLAY_BEGIN_TIME,A.PLAY_END_TIME,B.BEGIN_TIME,B.END_TIME)='2' ");
				break;
			// 等待播放
			case ORDER_STUTAS_WAITPLAY:
				sql.append("A.ORDER_STATUS = '4' AND A.order_type<>4  AND judegNowInTargetSection(A.PLAY_BEGIN_TIME,A.PLAY_END_TIME,B.BEGIN_TIME,B.END_TIME)='1' ");
				break;
			// 已停播
			case ORDER_STUTAS_STOPPLAY:
				sql.append("A.ORDER_STATUS='4' AND A.order_type=4 ");
				break;
			// 已关闭
			case "7":
				sql.append("(A.ORDER_STATUS='7' or (A.ORDER_STATUS='4' AND A.order_type<>4 AND judegNowInTargetSection(A.PLAY_BEGIN_TIME,A.PLAY_END_TIME,B.BEGIN_TIME,B.END_TIME)='3')) ");
				break;
			// 其它状态
			default:
	...
	public List<AdDto> queryADForBuy(AdDto adDto){
		List<Object> params = new ArrayList<Object>();
		StringBuffer sql = new StringBuffer(
					 "SELECT A.ID ADVERTISER_ID,B.DEVICE_AD_NAME name,B.DEVICE_ADDRESS MAILING_ADDRESS,B.DEVICE_ID,B.device_code,C.AD_LENGTH,C.PLAY_NUMBER allPlayNumber,C.AD_PRICE price,C.BEGIN_TIME begin_time,C.END_TIME end_time, "
		...
		if(addr!=null && !addr.trim().equals("")){
			if(addr.split("\\*").length>=4){
				sql.append(" and substring_index(B.DEVICE_ADDRESS,\"&\","+1+")=?");
	   		}else{
	   			sql.append(" and substring_index(B.DEVICE_ADDRESS,\"*\","+addr.split("\\*").length+")=?");
	   		}
			//sql.append("AND B.DEVICE_ADDRESS LIKE ? ");
			params.add(addr);
		}
		if(!RegUtil.getUtil().isNull(adDto.getAdName())){
			sql.append(" and (B.DEVICE_AD_NAME like CONCAT('%',CONCAT(?,'%')) || B.device_code like CONCAT('%',CONCAT(?,'%'))) ");
			params.add(adDto.getAdName());
			params.add(adDto.getAdName());
   	 	}
		//类别
		if(!RegUtil.getUtil().isNull(adDto.getDevice_industry())){
			sql.append(" and B.DEVICE_INDUSTRY=? ");
   		 	params.add(adDto.getDevice_industry());
		}
		// 判断是否根据价格查询
