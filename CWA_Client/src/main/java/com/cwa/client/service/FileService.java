...
public class FileService implements Constant{
	 ...
	 * 图片基础路径
	 */
	private static final String IMG_BASE_UPLOAD_PATH = "baseUploadPath";
	
	/**
	 * 文件基础路径
	 */
	private static final String BASE_UPLOAD_PATH = "STREAM_FILE_REPOSITORY";
	...
	public RuturnMessageDto uploadPicFile (HttpServletRequest req,MultipartFile file,String path) throws Exception{
		...
		try {
			...
			logWriteUtil.writeLog(LOGTYPE_SERVICE, "File type is "+file.getContentType(), LOGLEVEL_INFO, FileService.class);
			Map<String, Object> mapJudeg = Tools.judgeImage(bytes);
			if (!(boolean) mapJudeg.get("isImage")) {
				logWriteUtil.writeLog(LOGTYPE_SERVICE, "File type not picture", LOGLEVEL_INFO, FileService.class);
				rt.setMsg(reqCt.getMessage("file.tip.fileNotPic"));
			...
			/*if(!RegUtil.getUtil().isImage(file.getContentType())){
				logWriteUtil.writeLog(LOGTYPE_SERVICE, "File type not picture", LOGLEVEL_INFO, FileService.class);
				rt.setMsg(reqCt.getMessage("file.tip.fileNotPic"));
				return rt;
			}*/
			...
			// 判断目录是否存在
			String dirPath = Encypter.getValueByKey(IMG_BASE_UPLOAD_PATH) + path;
			...
			// 文件名
			String fileName = accountId+userType+new Date().getTime()+"."+file.getContentType().split("/")[1];
			// 文件路径
			String filePath = dirPathDate + fileName;
			// 转存文件
			if(file.getContentType().toLowerCase().indexOf("gif")<0){
	...
	public void loadPicFile(HttpServletResponse res,String filePath) throws Exception{
		...
		try {
			...
			ips = new FileInputStream(new File(Encypter.getValueByKey(IMG_BASE_UPLOAD_PATH) +filePath));
	 ...
	 * @return
	 */
	public RuturnMessageDto uploadFile(HttpServletRequest req,MultipartFile file,String path,Long maxSize){
		// 返回的消息对象
		RuturnMessageDto rt = new RuturnMessageDto();
		// 语言包
		RequestContext reqCt= new RequestContext(req);
		try {
			...
			if (bytes.length > maxSize) {
				logWriteUtil.writeLog(LOGTYPE_SERVICE, "File too big size is "+bytes.length+",max size is "+maxSize, LOGLEVEL_INFO, FileService.class);
				rt.setMsg(reqCt.getMessage("file.tip.fileTooBig")+(maxSize/1024/1024)+"m");
				return rt;
			}
			logWriteUtil.writeLog(LOGTYPE_SERVICE, "File size is "+bytes.length, LOGLEVEL_INFO, FileService.class);
			// 判断目录是否存在
			...
			String accountId = accountIdField.get(sessionUser).toString();
			// 文件名
			String fileName = accountId+userType+new Date().getTime()+"."+file.getOriginalFilename().split("\\.")[file.getOriginalFilename().split("\\.").length-1];;
			// 文件路径
			String filePath = dirPathDate + fileName;
			// 转存文件
			file.transferTo(new File(filePath));
	 ...
	 * @param contentType
	 */
	public void loadFile(HttpServletResponse res,String filePath,String contentType){
		ServletOutputStream out = null;
		FileInputStream ips = null;
		try {
			// 将文件的流返回前台
			...
			byte[] buffer = new byte[1024 * 10];
			out = res.getOutputStream();
			String filePath2 = Encypter.getValueByKey(BASE_UPLOAD_PATH) + "/" +filePath;
			logWriteUtil.writeLog(LOGTYPE_SERVICE, "File path is "+filePath2, LOGLEVEL_INFO, FileService.class);
			ips = new FileInputStream(new File(filePath2));
			while ((len = ips.read(buffer)) != -1) {
				out.write(buffer, 0, len);
			}
			out.flush();
		} catch (Exception e) {
			logWriteUtil.writeLog(LOGTYPE_SERVICE, "load file exception,"+e.getMessage(), LOGLEVEL_ERROR, FileService.class);
			e.printStackTrace();
		} finally {
			try {
				if (out != null)
