...
public class AdvertiseInfoService implements Constant{
    ...
    private PageVars getVars(AdvertiseInfoDto dto,int type){
    	 ...
    	 if(type==1){
    		 ...
    		 sbStr.append("select A.id,B.DEVICE_AD_NAME SHOP_NAME,B.device_code,B.DEVICE_ID,B.DEVICE_ADDRESS MAILING_ADDRESS,C.AD_PRICE ,C.AD_LENGTH,C.PLAY_NUMBER,CONCAT(C.BEGIN_TIME,\"-\",C.END_TIME) as idle_time, "
    				 ...
    				 +"C.play_number-ifnull(F.play_number,0) toDaySell,"
    				 + "if(D.id is not null,true,false) ifSubBuy "
    				 +"from tb_advertiser A inner join tb_advertiser_device B on A.id=B.ADVERTISER_ID "
    				 + "inner join tb_device_vedio_plan C on B.DEVICE_ID=C.DEVICE_ID "
    				 +" left join tb_device_vedio_count F on F.device_id=C.device_id and F.begin_time=C.begin_time and F.end_time=C.end_time and date_format(F.play_date,'%Y-%m-%d')=date_format(now(),'%Y-%m-%d') "
    				 + "left join tb_cart D on D.device_id=C.device_id and D.account_id=:account_id where A.AUDIT_STATUS=2 and trim(B.device_id)<>'' and C.ad_sell_status=1");
    	 }
    		 ...
    		 for (AdvertiseInfoDto dto : list) {
				if((dto.getId()+"").equals(strs1[i])&&dto.getDevice_id().equals(strs2[i])/*&&dto.getIdle_time().equals(strs[i])*/){
					...
					for (int j = 0; j < count; j++) {
						...
						dto.setUnified_serial_number((j<10?"0":"")+j);
						BeanUtils.copyProperties(newDto, dto);
						newDto.setBuyCount(count);
						list1.add(newDto);
					}
