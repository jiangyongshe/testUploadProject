...
public class AdOrderController extends BaseController<GobalRespParameter> implements Constant{
	...
	public void queryOrder(HttpServletRequest req, HttpServletResponse res) throws Exception{
		...
		try {
			...
			if(util.isNumber(pageSize)&&util.isNumber(pageNo)){//判断参数是否正确
				...
				dto.setUnified_serial_number(getParameter(req, "unifiedOrderNo"));
		...
		try {
			if(map!=null){
				...
				if(orderList.size()>0){
					...
					}else if(parameter.getMsg().equals(RespCodeEnum.order_payMoreBig_error.getCode())){
	...
	public void deleteOrder(HttpServletRequest req, HttpServletResponse res) throws Exception{
		RuturnMessageDto parameter=new RuturnMessageDto();
		parameter.setSuccess(false);
		Map<String,String[]> map = getAllParameter(req,"deleteOrder.do","orderNo");
		RequestContext reqCt= new RequestContext(req);// 语言包
		CustomerDto customerDto=(CustomerDto)req.getSession().getAttribute(SESSION_USER);//session得到user信息
		try {
			if(map!=null){
				orderService.deleteOrderByOrderNo(map.get("orderNo")[0],customerDto.getAccount_id());
				parameter.setSuccess(true);
				parameter.setMsg(RespCodeEnum.SUCCESS.getCode());
				parameter.setData(reqCt.getMessage("pay.common.order.deleteSuccess"));
			}else{//请求参数无效
				parameter.setData(reqCt.getMessage("common.exception.parameter"));
			}
		} catch (Exception e) {
